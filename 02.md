...
Keyset IDs are 33 byte hex strings with a version byte (two hexadecimal characters). The currently used version byte is `02`.

For V3 keysets, all public keys used in the keyset (e.g., in the response to `/keys/` or `/keys/{keyset_id}`) and for deriving the keyset ID **MUST** be points on the `G2` group of the BLS12-381 curve.

The derivation process for V3 keyset IDs is identical to V2, but uses a version prefix of `02` to distinguish them as BLS12-381 keysets.

To derive the keyset ID of a V3 keyset, execute the following steps:
...
```python
def derive_keyset_id_v3(keys: Dict[int, PublicKey], unit: str, input_fee_ppk: Optional[int], final_expiry: Optional[int]) -> str:

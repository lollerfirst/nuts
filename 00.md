...
### Keyed-verification (Legacy)

Legacy Cashu tokens (secp256k1) use keyed-verification. Verification of a signature `C` for a secret `Y` requires knowledge of the mint's private key `k`:
`k * Y == C`

Because only the mint (`Bob`) possesses the private key `k`, verification is not publicly possible without the mint's involvement or a DLEQ proof provided by the mint.

# BLS12-381 key exchange

BLS12-381 keysets, referred to as `v3` keysets, utilize the BLS12-381 curve.

## Hash-to-curve

`v3` keysets utilize the BLS12-381 curve and follow the [RFC 9380](https://datatracker.ietf.org/doc/html/rfc9380) standard for mapping messages to the `G1` group, which handles the curve's cofactor. The implementation relies on standard libraries (e.g., `pyblst`) to perform `hash_to_group` operations to map arbitrary byte strings to valid, uniformly distributed points on the curve.
...
The signature `C` on a secret `Y = hash_to_curve(x)` is verified by checking:
`e(C, G2) == e(Y, K2)`
...
This verification is public; anyone with access to the mint's public key `K2` can verify the signature `C` for a given secret `Y` without requiring additional DLEQ proofs.
